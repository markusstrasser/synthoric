...
export default {
  description: 'A solution for a given task/exercise',
  prompt: 'You are responding with a solution and review for the following Exercise or Task:',
  params: z.object({
    task: z.any(),
  }),
  schema: z
      ...
      stepsNested: z
        .array(
          z.object({
            step: z
              .string()
              .describe('A high-level step or phase in the overall solution process.'),
            subSteps: z.array(
              z.object({
                title: z
                  .string()
